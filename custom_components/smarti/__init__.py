...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry):
    ...
    # Forward the setup to the updater platform
    hass.async_create_task(
        hass.config_entries.async_forward_entry_setup(entry, "updater")
    )
...
async def async_unload_entry(hass: HomeAssistant, entry: ConfigEntry):
    ...
    await hass.config_entries.async_forward_entry_unload(entry, "updater")
...
async def async_migrate_entry(hass: HomeAssistant, entry: ConfigEntry):
    ...
    # For now, we'll just ensure the entry version is up to date.
